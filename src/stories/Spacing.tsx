...
function SpacingRow({ step, utility }: ISpacingStep) {
  ...
  return (
    <tr>
      ...
      <td className="py-3.5 align-middle type-body-m text-text-primary">
        {rem || "—"}
      </td>
      <td className="py-3.5 align-middle">
        <div
          ref={boxRef}
          aria-hidden="true"
          className={`${utility} h-4 rounded-sm bg-brand`}
        />
      </td>
    </tr>
  );
...
export function SpacingTokens() {
  return (
    <section
      aria-labelledby="spacing-tokens-title"
    ...
    >
      ...
      <p className="type-body-m text-text-secondary max-w-149">
        This project has no custom spacing tokens — every gap, width, padding,
        and margin comes straight from Tailwind&apos;s own scale, each step a
        multiple of --spacing (e.g. gap-4 is calc(var(--spacing) * 4)). A
        handful of steps below, read live from the DOM.
      </p>
