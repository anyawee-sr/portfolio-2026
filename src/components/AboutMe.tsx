      ...
      <div className="surface-texture mx-auto sm:max-w-190 shadow-2xl -rotate-6 translate-x-4 translate-y-10">
        <div className="relative flex flex-col border border-brand/45 m-4 sm:m-10 sm:min-h-200 md:min-h-225">
          <div className="border-b border-brand/40 px-2 pt-14 text-center">
            ...
            <p className="type-body-s mx-auto mt-2 max-w-170 text-brand uppercase lg:text-nowrap">{aboutMe.subcopy}</p>
          </div>
          ...
          <dl className="about-fields -mr-px">
            {aboutMe.fields.map((field) => (
              ...
              >
                <dt className="type-caption text-brand uppercase">{field.label}</dt>
            ...
            <div className="col-span-12 border-r border-b border-brand/40 pl-8 pr-4 md:border-b-0 md:col-span-9 md:p-0">
              <ul className="flex flex-col">
                {aboutMe.contact.rows.map((row, i) => (
                  <li key={row.label} className="flex items-center gap-2 md:px-2 md:border-b border-brand/40">
                    <CheckBox checked index={i} />
                    <span className="type-caption min-w-20 shrink-0 text-brand uppercase">{row.label}</span>
                    <a
                      href={row.href}
          ...
          <div className="-mr-px -mb-px hidden md:block">
            <div aria-hidden="true" className="grid h-7.5 grid-cols-12 border-t border-brand/40">
            ...
            <div className="grid grid-cols-12">
              <div className="col-span-4 border-r border-b border-brand/40 p-2">
                <dl className="flex flex-col">
                  <dt className="type-caption mb-3 text-brand uppercase">{aboutMe.lookingFor.label}</dt>
                  {aboutMe.lookingFor.options.map((option, i) => (
                    <dd key={option.label} className="mb-1 ml-4 flex items-center gap-2 last:mb-0">
                      <CheckBox checked={option.checked} index={i + 2} />
                      <span className="type-caption text-brand uppercase">{option.label}</span>
                      {!option.checked && <span className="sr-only"> — not looking for</span>}
                    </dd>
                  ))}
                </dl>
              </div>
          ...
          <div className="absolute right-6 bottom-12 hidden md:block">
            ...
            <div className="absolute -bottom-10 right-18 flex -translate-x-1/2 rotate-6 flex-col items-center text-center">
              <span className="type-body-m font-typewriter text-text-primary uppercase">{aboutMe.resume.prefix}</span>
              <a
